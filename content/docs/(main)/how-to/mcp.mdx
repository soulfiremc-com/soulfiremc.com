...
MCP (Model Context Protocol) is an open standard that lets AI assistants interact with external tools and services.
SoulFire exposes its full API as MCP tools, so an assistant can control bot instances, execute commands,
manage configurations, and more through natural language.

Make sure SoulFire is already installed and running before you start.
If it isn't, head to [Start Here](/docs/start-here) first.
...
Any MCP-compatible client can connect to SoulFire. These are the settings you need:
...
SoulFire exposes more than 40 MCP tools, grouped into a few categories:
