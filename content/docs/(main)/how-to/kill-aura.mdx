---
title: Kill Aura
description: Configure targeting ranges, cooldowns, protected players, and wall checks for combat tests.
icon: Sword
---
...
`kill-aura` makes a bot look at and attack nearby targets within configured ranges.
...
| `check-walls` | Ignores targets behind walls when enabled |
...
Set the ranges in this order:

- Use the largest value for `look-range`
- Use a smaller value for `swing-range`
- Use the smallest value for `hit-range`
...
<Steps>
  ...
  <Step>
    ...
    Test targeting and attack timing with one bot before you add more bots.
  </Step>
</Steps>
...
If the value is too small, the bot can react late.
If the value is too large, the bot can react to distant entities.
...
With `check-walls` enabled, the bot avoids targets that it cannot clearly see or reach.
...
Make sure that `check-walls` is enabled.
Then decrease the attack ranges.
