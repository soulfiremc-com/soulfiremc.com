...
While a CLI is similar to a dedicated server, it does not accept connections from other clients
and its startup flags configure the local integrated server.

## How to start the server in dedicated mode

Make sure your machine is using Java 25.
Next you can download the latest dedicated server `.jar` binary from the [download page](/download)
...
You can also explicitly enable/disable
...
It allows anyone with access to it to connect to your server through the GUI or CLI and control bots with your permissions.
...
You can find examples of how to use those Docker images in the [Docker guide](/docs/how-to/docker).

## What now?

You've successfully set up a dedicated server. Now connect to it from the GUI, CLI, or SDK and try running some bots.
Dedicated servers use the same underlying code as integrated servers, so the experience should be the same.
Servers do most of the heavy work. They run bot connections, check proxies, authenticate accounts, and more.
