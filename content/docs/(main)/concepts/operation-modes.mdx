...
In SoulFire, the client tells the server which bots should run and the server owns those bot connections.

## Integrated server (local)

The GUI/CLI comes pre-installed with a SoulFire server.
This means that you can run bots on your machine without needing to set up a dedicated server.
...
If you want to run bots on a remote machine, set up a dedicated server.
...
This is useful if you want to run bots on a remote server that you control from another machine.
