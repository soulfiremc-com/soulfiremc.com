---
title: Triggers
description: Learn when each SoulFire scripting trigger runs and what data it provides.
icon: Zap
---

import { Callout } from 'fumadocs-ui/components/callout';

Triggers start SoulFire scripts.
When a trigger fires, SoulFire runs the connected execution path.
...
- Event triggers use a buffer. Under heavy load, SoulFire can discard extra events when the buffer is full.
- `On Pre Entity Tick` and `On Post Entity Tick` run on the tick thread. Expensive nodes can delay the game loop.
- `On Interval` does not output a bot. If you need to act on bots, use `Get Bots` and `For Each Bot`.

## Bot connection triggers

### On Bot Init

Fires when the bot connection starts, before the player object is ready.
...
Use it for setup, startup logs, and shared data.
...
`On Script End` runs during shutdown.
SoulFire waits up to **5 seconds** for this trigger, so keep cleanup work short.

## Tick triggers

### On Pre Entity Tick

Fires before entity movement in each tick.
It runs directly on the tick thread.
...
Fires after entity movement in each tick.
It runs directly on the tick thread.
...
Use this for logic that needs the world state after movement.
For example, use it for a combat action after movement.
...
- Use `On Interval` for a task that runs at a fixed interval.
...
- Putting all behavior in one large chain. One script can use several triggers.
