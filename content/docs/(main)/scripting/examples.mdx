...
These examples use the current SoulFire scripting system.
Each example explains its graph and nodes.
...
This example shows the basic trigger-to-action pattern.
...
For more commands, add more conditions or connect the result to a `Switch` node.

## 3. Poll all bots every few seconds

**Goal:** Run the same check for every connected bot on a timer.

`On Interval` does not provide a bot, so this example gets the bot list first.
...
1. Use `String Contains` to select messages for the model.
