...
<Callout type="info">
  Build SoulFire scripts by connecting nodes in the visual editor.
  Triggers, data flow, and limits control how each script runs.
</Callout>

<Callout type="info">
  Use scripting when the available nodes can express your workflow.
  Use [Development](/docs/development) for Mixins, packet handling, custom settings pages, or direct object access.
</Callout>
...
Build scripts in the **Scripts** tab of an instance.
Each script belongs to one instance and can be active or paused.
An active script listens for events.

## How scripts actually run

SoulFire scripts are event-driven.
A script waits for a trigger, then runs the connected nodes.
