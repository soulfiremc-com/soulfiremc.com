...
This page prepares a workspace for SoulFire plugin development.
The finished workspace can build and debug a plugin and show the current API.
...
<Callout type="info">
  ...
  An older JDK can cause build and toolchain errors.
</Callout>
...
Use the template for project structure.
Use the SoulFire repository and Javadocs for current API behavior.
...
- Read the current [CodeMC Maven metadata](https://repo.codemc.org/repository/maven-public/com/soulfiremc/mod/maven-metadata.xml) before you select a version.
...
Before you copy low-level code, compare it with the current Javadocs and source.
