...
Durable tasks run on the SoulFire server near the Minecraft game loop.
Use them for work that needs progress, cancellation, retries, reconnect rules, or shared control.
...
an origin. SoulFire transforms the blueprint on the server and places blocks
from the bottom up. It navigates between positions, selects exact materials,
and checks each block state.
