...
Durable tasks execute on the SoulFire server beside the Minecraft game loop.
Use them for work that has progress, cancellation, retries, reconnect policy,
...
- Clearing an inclusive cuboid with bounded excavation
- Building transformed and partitioned inline schematics
...
- Executing an exact number of villager trades
- Following a moving entity
- Chasing and attacking an entity
- Leading moving targets with bows and crossbows
...
- Exploring a bounded frontier with coordinated fleet claims
- Stashing and withdrawing transactional item batches
- Maintaining bounded inventory loadouts from storage
...
SoulFire chooses the nearest remaining block, plans each route beside the
game loop, selects tools through the pathfinder, and never mines outside the
requested cuboid.
...
The server rejects cuboids larger than 32,768 blocks and regions whose chunks
are not loaded yet. `maximumBlocks` or `maximum_blocks` bounds one run. The
typed result distinguishes a cleared area, a reached block budget, and a
region with no reachable blocks. It also reports skipped and unreachable
positions. Cancellation or suspension stops the active route and releases
movement, rotation, hand, and inventory resources.

## Build an inline schematic

`build` accepts a language-native blueprint made of block offsets relative to
an origin. SoulFire transforms the whole blueprint on the server, orders
placements from the bottom up, navigates between them, selects exact
materials, and verifies the resulting block state.
...
Offsets and directional properties rotate and mirror together. A substitution
list is ordered, so the first available replacement wins. Existing exact
states count as complete without consuming material. Every requested block
gets an outcome such as placed, already correct, missing material,
unreachable, unsupported, state mismatch, or blocked by an incorrect block.
...
Pathfinding may place navigation scaffolds when `allowPlacing` or
...
The task refuses to continue if the merchant changes that offer, runs one
trade at a time so the count is exact, and stops before taking an output when
the inventory cannot hold it.
...
Combat automatically ranks and equips the strongest matching melee weapon,
respects attack cooldown, chases through Pathfinder v2, and restores the
previous hotbar selection when the task stops.

## Use a bow or crossbow

Use `rangedAttack` or `ranged_attack` when a bot should keep its distance and
fire at a specific observed entity. The task can lead a moving target,
compensate for arrow gravity, strafe while aiming, and move back into its
configured range when the target closes in or escapes.
...
The task accepts bows and crossbows, finds compatible ammunition through the
vanilla projectile rules, and reports distinct outcomes for a defeated
target, a lost target, a shot limit, missing ammunition, and a missing
weapon. The target reference is scoped to the current bot connection. A
...
`clear_seconds`. Set target or attack limits when the task should end on a
...
The breeding task selects two compatible adult animals in a bounded area,
finds an inventory item accepted as food by both, walks to each animal, and
feeds them:
...
count. Minecraft may still delay or prevent offspring after both parents enter
love mode.
...
finite rebalance limit. Transfers are preflighted as batches and use partial
movement deliberately, so a temporarily empty chest produces a typed
container-exhausted result for one-shot work instead of corrupting menu
state. Results report final counts, whether each requirement is satisfied,
...
the work should survive the SDK process.
