...
Node-only JSON store to keep one worker state across restarts:
...
evidence. Durable tasks use stable idempotency keys and deadlines.
After a restart, the worker attaches to existing server work instead of
duplicating it.
