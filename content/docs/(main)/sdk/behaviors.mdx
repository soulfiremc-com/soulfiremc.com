...
description: Compose SoulFire tasks into cancellable application workflows with Effect, Promises, or asyncio.
icon: Workflow
---
...
Behavior combinators coordinate application-level workflows. Use them to
decide what a bot should do next while durable tasks keep movement, combat,
inventory, and survival timing beside the Minecraft game loop.
...
| `parallel` | Independent work may run concurrently |
| `race` | The first successful strategy should win |
| `repeat` | A step should run a fixed number of times |
| `retry` | A transient failure may recover |
...
cancels the running operation. The Promise timeout does not depend on a custom
behavior honoring cancellation before it returns to the caller, though native
SDK operations do receive the linked signal.
