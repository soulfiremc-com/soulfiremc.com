...
description: Observe, inspect, and inject version-dependent Minecraft packets through the SoulFire SDK.
icon: Binary
---
...
SoulFire exposes its active native Minecraft packet codec as an advanced SDK
module. Use it when a typed high-level operation does not exist yet, when a
server uses a custom packet extension, or when you are diagnosing protocol
behavior.

Prefer the typed world, inventory, chat, action, and task APIs for normal
automation. Raw packet code is version-dependent and can put a bot connection
...
<Callout type="warn">
  Raw packet sending requires the `RAW_PROTOCOL` instance permission. SoulFire
  grants it only to administrators by default, limits packets to 1 MiB and 20
  sends per second per user and bot, and writes every send to the server log.
</Callout>

Raw observation and injection do not bypass plugin RPCs, task ownership, or the
normal typed SDK. A SoulFire plugin should expose a typed RPC when an operation
