...
description: Move Mineflayer automation to SoulFire's native SDK, durable tasks, synchronized state, and plugin RPC model.
icon: Replace
---
...
SoulFire covers the same automation problems as Mineflayer through a
server-authoritative SDK. It does not emulate Mineflayer's API. Migrate each
concept to the native SoulFire object model so the application gains durable
tasks, reconnect-safe state, fleet operations, permissions, and typed plugin
capabilities.
...
Each concept maps to a native API, a language-level convenience, a durable
server task, a plugin extension, or the permission-gated protocol escape
hatch.
