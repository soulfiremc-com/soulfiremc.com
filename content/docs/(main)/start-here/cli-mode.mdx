...
<Callout type="info" emoji="ℹ️">
  ...
  Guide](/docs/start-here/gui-mode) instead.
</Callout>

CLIs are for power users who prefer a keyboard or want to automate bots.
SoulFire has those types of advanced users covered with its CLI.
...
Next you can download the latest client `.jar` binary from the [download page](/download)
...
Now decide whether the bots run on your machine or on a remote dedicated server.
...
For a full list, visit the [CLI flags page](/docs/reference/cli-flags)
...
Make sure to replace `$RAM` with the amount of ram you want to allocate to the CLI instance.
The command above imports the accounts in `accounts.txt`, connects them to a local Minecraft server on port 25565,
and starts every imported bot with a delay between 1 and 3 seconds.
...
If you want to know more about commands, visit the [Commands](/docs/reference/commands) reference.
